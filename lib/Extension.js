...
function Extension(connection, config) {
    ...
    function createJob(name, options, callback) {
        ...
        options = _.defaults(options, {
            ...
            removeOnCompletion: !options.due.hasOwnProperty('fromCronString')
        });
    ...
    function runJob(job) {
        if (!cancelling[job.name]) {
            ...
            var handlers = jobHandlers[job.name] || jobHandlers[job.group];
            if (!handlers) {
                logger.warn('Job run without handlers! You can add an "All" group handler to capture all jobs', job);
                return;
            }
